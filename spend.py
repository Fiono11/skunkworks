...
def prove(pk,C_in,k,a,kappa,sk,delta,seed=None,aux1=Scalar(0),aux2=Scalar(0)):
    ...
    r_a1 = Scalar(5)
    r_a2 = Scalar(5)
    r_sk = [Scalar(5) for _ in range(M)]
    r_d = Scalar(5)
    sL = ScalarVector([Scalar(5) for _ in range(M*RING)])
    sR = ScalarVector([Scalar(5) for _ in range(M*RING)])
...
def verify(proof,pk,C_in,C_out):
    ...
    data = []
    W = ScalarVector([])
    print "scalar: ", list(bytearray.fromhex(str(Scalar(1000))))
    ...
    data_bytes = []
    for i in range(len(data)):
        data_bytes.append([list(bytearray.fromhex(str(data[i][0]))), list(bytearray.fromhex(str(data[i][1])))])
